...
5. 工具结果以 text/image segments 通过完成 Hook，随后写入 transcript；纯文本只存 `content`，多模态结果额外存 `segments`。
...
- Hook 可返回控制字段和输出意图。
- 进程 Hook 可用 `message.segments` 替换当前消息；工具完成 Hook 的修改会进入 transcript 和后续 LLM 请求。
- Hook 不直接发平台消息。
