...
Chat History 库只保存原始来源。主库 `media_history` 保存历史记录内部 ID、平台/scope/消息 ID、媒体位置及媒体 ID，并以触发器同事务维护 `chat_history` owner 引用。入站实际消费和 get_media 共用导入与关联规则；文本工具结果中的 ID 本身不建立 Transcript 媒体引用。删除历史成功后释放关联；启动和媒体清理前按主库已有关联分页核对历史 owner，消息不存在或内部 ID 已替换才释放，历史库故障保守停止。关联替换使用新的 owner ID，避免延迟对账删掉新关联。最后引用释放后继续遵循 1 小时孤儿宽限期，不扫描/下载全部历史。
