...
- `docs.en/` / `README.md` / `CHANGELOG.en.md`：英文用户文档镜像，由 GitHub Actions 自动翻译生成；不要手动修改。
...
- `internal/tool/discover.go`：`discover_tool` 内置发现工具；有 `name`/`names` 时普通工具仅返回“已发现工具”文本并把完整 schema 留在结构化 Data 供 Agent 注入 top-level tools，外置 AgentSkill/Go skill 返回 markdown/ELyph detail；查询说明型 AgentSkill 会激活 `agent_skill` 元工具，查询工具化 AgentSkill 会注入其 top-level schema，查询 Go skill 会按需激活 `go_skill_run`。
...
- `internal/toolrun/`：工具调用中间层；维护 session 工具缓存、native/Elwisp 工具视图、foreground-only 可用性过滤、命名解析、权限风险确认、工具自定义确认详情、tool call 生命周期编排、批量工具预览和失效提示；后台 session 不注入默认 `discover_tool`，执行前可注入工具 context，Elwisp 外部工具通过 HTTP JSON POST 执行。
...
- `internal/tool/builtin/file_tools.go`：文件读写工具包装层；`read_file` 返回带行号文本和文件哈希，支持 grep 子串搜索，路径基于 workspace 或后台 sandbox 解析，读取受保护文件会返回工具建议 warning；`edit_file` 支持行编辑、match/anchor、创建新文件、unified diff 和风险确认详情，直接修改受保护文件会被拒绝；底层读写编辑能力来自 `internal/utils/fileops`。
...
- `internal/tool/skill/descriptor.go`：skill 描述对象；让 AgentSkill/Go skill 可被 `discover_tool` 查到详情，按结构化 detail 暴露内容、格式和规则卡；说明型 AgentSkill 会激活 `agent_skill` 元工具。
- `internal/tool/skill/scanner.go`：skill 文件系统扫描与 reload；主程序默认根目录为配置目录下 `skills/`；AgentSkill 使用 `agent/<skill>/SKILL.md`，可选 `SKILL.elyph` 覆写 Agent 可读说明，并只读取根目录 `ELBOT_SKILL.toml` 决定是否注册为普通工具；Go skill 必须使用 `go/<skill>/SKILL.elyph`，可选 binary；同步新增/删除 skill 并更新 catalog。
- `internal/tool/skill/manager.go`：Skill manager；维护 catalog/scanner/registry，并提供延迟后台扫描与 `discover_tool` 首次使用兜底加载。
- `internal/tool/skill/runner.go`：Go skill 隐藏包装工具；`go_skill_run` 选择 Go skill binary 并把必填 `payload` 对象 JSON 写入 stdin；执行错误会区分启动/超时/进程失败并回传 stdout/stderr，风险按目标 skill 的 `risk` 评估。
...
- `internal/platform/qq-onebot/`：QQ OneBot v11 正向 WebSocket 适配；负责入站图片解析、私聊超级管理员 MessageEvent FileSegment 文件下载到 sandbox 平台目录、纯文件保存/过大提示、群聊触发、引用上下文和文本/媒体输出。
- `internal/platform/qqofficial/`：QQ 官方机器人 C2C 单聊适配。
- `internal/platform/telegram/`：Telegram Bot API long polling 适配。
...
- `internal/session/expiration.go`：Session 闲置过期策略；按群聊/私聊和普通用户/超管配置选择 TTL，并删除过期的当前 Session。
...
4. 若有新功能或者修改旧功能，需要更新对应readme（用户侧）和CHANGELOG.md
5. 整体语言用简体中文。
