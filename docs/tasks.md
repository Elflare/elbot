...
- [ ] 后续用更模块化的补全服务替代当前 Agent 辅助方法，由 app 层装配并注入具体平台。
...
- [x] 支持基础命令自动补全能力。
- [ ] 将 CLI 补全改为独立补全服务或平台能力装配，不再通过 Agent 辅助方法暴露。
- [ ] 支持 fish/readline 风格的候选提示、上下选择和参数补全。
...
- [x] 常驻记忆支持增删改查。
- [ ] 长期记忆暂定使用 SQLite 存储，由 LLM 通过工具主动增删改查。
- [ ] 长期记忆与 Skill 区分：反复使用且具有固定流程的能力应实现为 Skill。
