...
| 命令 | 作用 |
| --- | --- |
| `/requests` | 查看当前进程中的 active request，包括每个 turn 的运行阶段（preparing/llm/tool/sending）和阶段耗时。 |
...
`workspace` 工具用于设置当前 Session 的共享工作目录。设置后，所有需要路径的工具都会基于该目录解析相对路径；不传参数时返回当前 workspace，`reset=true` 时重置为默认工作目录。cron/Elnis 后台任务不使用 workspace，仍限制在各自 sandbox 内。
