...
- `[elnis].enabled=false` 时不启动 Elnis HTTP runtime。
- token 原文不写入配置，推荐放在系统环境变量或配置目录 `.env`。
- `token_env` 可以写多个环境变量名，按顺序尝试。
- Elwisp 默认启用；没写 `[elnis.elwisps.<name>]`、写了但没写 `enabled`、或写了 `enabled=true` 都会接收。
...
| `tool_list_names` | 否 | 后台任务预加载的 ElBot 工具名；`discover_tool` 会被忽略。 |
| `tools` | 否 | Elwisp 随事件声明的工具。当前仍是开发中能力，不建议依赖。 |
...
- token 原文不写日志。
- Elwisp 不能直接发送平台消息。
- Elwisp 不能绕过 Tool Runtime 和 Security Policy 调用 ElBot 工具。
- Elwisp 声明的 `tools` 当前仍是开发中能力，不应作为稳定接口依赖。
