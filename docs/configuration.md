...
| `skills/` | 用户侧 Skill 目录，默认位于配置目录下；当前子目录为 `skills/agent/` 和 `skills/go/`。AgentSkill 可在根目录放置 `ELBOT_SKILL.toml` 注册为普通工具。 |
...
首次自动生成的文件包括：`app.toml`、`providers.toml`、`state.toml`、`SOUL.md`、`memories.toml`、`elnis.toml`、`skills/agent/agent_skill_creator/SKILL.md`、`skills/go/write_elbot_hook/SKILL.elyph` 和 `.env.example`；同时会创建 `skills/`、`skills/agent/`、`skills/go/`、`plugins/` 和 `long_memory/` 目录。已有文件不会被覆盖。`elnis.toml` 默认 `enabled=false`，不会在首次运行时启动 HTTP 监听。Skill 扫描在启动后延迟执行，首次使用 `discover_tool` 时会兜底等待或触发扫描。
...
AgentSkill 默认只作为说明文档使用；若按文档执行脚本，风险由实际调用的 `shell` 等工具承担。若要把 `skills/agent/<skill>/` 注册成普通工具，在该 Skill 根目录添加 `ELBOT_SKILL.toml`，工具风险以其中的 `risk` 为准。默认生成的 `agent_skill_creator` Skill 可用于查看说明并辅助创建该文件：
