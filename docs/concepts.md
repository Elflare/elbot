...
Session 是 ElBot 的持久化会话单位，用来保存一次连续对话的上下文、模式、消息历史、压缩摘要和工具调用记录。
...
长对话会逐渐接近模型上下文窗口。上下文压缩会把较早的对话整理成摘要，并和最近的新消息一起组成后续发给 LLM 的上下文视图。

压缩不会删除原始消息，只会改变后续请求中使用的上下文内容。它可以自动触发，也可以由用户手动触发。
