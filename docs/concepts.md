...
- `@skill:<name>`：把指定 Skill 文档内容加入本轮用户消息；若 AgentSkill 带有合法 `ELBOT_SKILL.toml`，会注入该 Skill 的 top-level 工具 schema，否则仅作为说明文档使用。
...
超级管理员在平台里引用回复 LLM Cron 的通知消息时，会自动 resume 到对应后台 Session 继续对话。
...
- AgentSkill：放在 `skills/agent/<skill>/`，遵从或兼容 agentskills.io 风格 `SKILL.md`，也可用 `SKILL.elyph` 覆写 Agent 可读说明。默认是说明型 Skill，LLM 可按文档使用 shell、文件等通用工具处理附带脚本。
- 工具化 AgentSkill：AgentSkill 根目录存在合法 `ELBOT_SKILL.toml` 时，会注册为普通 top-level 工具。LLM 调用结构化参数，ElBot 按配置把参数翻译为命令行 argv 并在 Skill 目录执行命令，不通过 shell。
`ELBOT_SKILL.toml` 只在 `skills/agent/<skill>/` 根目录读取。最小示例：
