...
把流程拆成命名阶段，块内仍是普通语句。可选使用，不强制全包；顶层允许裸语句与 `step` 块混用。
...
- `step` 块只能出现在顶层，**不能嵌套** `step`。
- `step` 块**不能为空**，至少含一条有效语句。
- 同一文档内 `step` 名**不能重复**。
...
ELyph 当前是 v0.3，语法可能在未来版本中扩展。写 `SKILL.elyph` 或任务条时，建议保持结构简洁，避免依赖未在本文档中列出的实验性写法。
