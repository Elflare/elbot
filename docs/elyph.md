...
| 场景 | 文件 | 说明 |
| --- | --- | --- |
| 原生 El Skill | `SKILL.elyph` | 放在 skill 目录下，定义 skill 的核心逻辑。 |
...
- 原生 El Skill 的 `SKILL.elyph` 文件使用 `#skill` header。
- `SKILL.elyph` 放在 skill 目录（`skills/<name>/SKILL.elyph`）下。
- 如果同时存在 `SKILL.md`，ELyph 文件的描述会优先被 Agent 使用。
- Python skill 也可以用 `SKILL.elyph` 覆写 Agent 可读的说明。
...
ELyph 当前是 v0.2，语法可能在未来版本中扩展。写 `SKILL.elyph` 或任务条时，建议保持结构简洁，避免依赖未在本文档中列出的实验性写法。
