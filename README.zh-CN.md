...
### 8. Elnis / Elwisp / Elvena：让 ElBot 感知一切

传统 Agent 通常只会等待用户输入；Cron 只能响应时间。Elnis 让 ElBot 多了一种触发方式：外部事件。

Elnis 是 ElBot 的监听枢纽；Elwisp 是分布在各处的监听器，负责观测世界。

服务器变化、RSS 更新、Webhook 告警、日志变化、游戏事件、本地脚本甚至其他计算机设备，都可以送入 Elnis。ElBot 再统一决定是否记录、调用 LLM 分析或执行后台任务，最终通知用户。
