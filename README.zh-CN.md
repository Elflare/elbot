...
**可由 LLM 创建的 EL Skill**： ElBot 内置 `create_el_skill` 元工具，允许 LLM 将可复用经验沉淀为 EL Skill。创建时自动校验 ELyph 语法，可选附带 Go 源码并编译；创建后的纯 ELyph 文本或 Go 源码由统一的 `read_el_skill` / `modify_el_skill` 维护。
