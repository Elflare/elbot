...
**可扩展的 Hook 系统**： ElBot 内置 Hook Layer，可在 Agent 输入、LLM 请求、LLM 响应、平台发送、平台连接等关键事件点插入扩展逻辑。Hook 可以修改消息、追加输出意图、调用脚本等。规则 Hook 的 exec action 基于 stdio 通信，支持用任意语言或脚本编写插件；配合 [Elwisp](docs/elnis.md) 可实现条件触发 LLM、外部事件接入等强大功能。
